    ...
    const res = await fetch('/api/ai', {
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          existingAppointments: todayApts.map(a => ({ start: formatTime(a.startTime), end: formatTime(a.endTime), customer: a.customer.name })),
        },
      }),
    })
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchAiSlots} disabled={loadingSlots} className="flex items-center gap-2 px-3 py-2 text-sm bg-purple-50 text-purple-700 rounded-lg hover:bg-purple-100 transition-colors">
            <Sparkles size={14} />
            {loadingSlots ? 'Thinking...' : 'Suggest slots'}
          </button>
          <Link href="/schedule/new" className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-sm font-medium">
      ...
      {aiSlots.length > 0 && (
        <div className="mb-4 p-4 bg-purple-50 rounded-xl border border-purple-100">
          <p className="text-sm font-medium text-purple-800 mb-2 flex items-center gap-1"><Sparkles size={14} /> AI Suggested Open Slots for Today</p>
      ...
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setCurrentDate(d => { const n = new Date(d); n.setDate(d.getDate() - 7); return n })} className="p-2 hover:bg-gray-100 rounded-lg">
          <ChevronLeft size={18} />
        </button>
            ...
            return (
              <div key={day.toISOString()} className={`border-r border-gray-200 last:border-r-0 relative ${isToday(day) ? 'bg-purple-50/30' : ''}`}>
                ...
                {!loading && dayApts.map(apt => (
                  <Link key={apt.id} href={`/schedule/${apt.id}`}
      ...
      <div className="bg-white rounded-xl border border-gray-100">
        <div className="p-4 border-b border-gray-100">
          <h3 className="font-medium text-gray-900 flex items-center gap-2"><Calendar size={16} className="text-gray-400" /> All Upcoming Appointments</h3>
        </div>
            ...
            appointments.map(apt => (
              <div key={apt.id} className="p-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <select value={apt.status} onChange={e => handleStatusChange(apt.id, e.target.value)} className="text-xs border border-gray-200 rounded px-2 py-1 text-gray-600">
